...
KEpsilonLidDrivenCavitySolver3D::KEpsilonLidDrivenCavitySolver3D(std::size_t nx, std::size_t ny,
    ...
    : nx_(nx), ny_(ny), nz_(nz), lengthX_(lengthX), lengthY_(lengthY), lengthZ_(lengthZ),
      ...
      p_(nx * ny * nz, 0.0), k_(nx * ny * nz, 0.0), epsilon_(nx * ny * nz, 0.0), nut_(nx * ny * nz, 0.0),
...
double KEpsilonLidDrivenCavitySolver3D::u(std::size_t i, std::size_t j, std::size_t k) const {
    return u_[indexU(i, j, k)];
}
double KEpsilonLidDrivenCavitySolver3D::v(std::size_t i, std::size_t j, std::size_t k) const {
    return v_[indexV(i, j, k)];
}
double KEpsilonLidDrivenCavitySolver3D::w(std::size_t i, std::size_t j, std::size_t k) const {
    return w_[indexW(i, j, k)];
}
double KEpsilonLidDrivenCavitySolver3D::pressure(std::size_t i, std::size_t j, std::size_t k) const {
...
double KEpsilonLidDrivenCavitySolver3D::lidVelocityAt(double x, double y) const {
...
double KEpsilonLidDrivenCavitySolver3D::stableTimeStep() const {
    double maxNut = 0.0;
    for (double n : nut_) {
...
double KEpsilonLidDrivenCavitySolver3D::uAt(long long i, long long j, long long k) const {
...
double KEpsilonLidDrivenCavitySolver3D::vAt(long long i, long long j, long long k) const {
...
double KEpsilonLidDrivenCavitySolver3D::wAt(long long i, long long j, long long k) const {
...
double KEpsilonLidDrivenCavitySolver3D::pAt(long long i, long long j, long long k) const {
    long long ci = i;
    long long cj = j;
    long long ck = k;
    if (ci < 0) {
...
double KEpsilonLidDrivenCavitySolver3D::nutAt(long long i, long long j, long long k) const {
...
double KEpsilonLidDrivenCavitySolver3D::maxDivergence() const {
    double maxDiv = 0.0;
    for (std::size_t k = 0; k < nz_; ++k) {
        for (std::size_t j = 0; j < ny_; ++j) {
            for (std::size_t i = 0; i < nx_; ++i) {
...
std::vector<double> KEpsilonLidDrivenCavitySolver3D::applyLaplacian(const std::vector<double>& x) const {
...
double KEpsilonLidDrivenCavitySolver3D::dot(const std::vector<double>& a, const std::vector<double>& b) {
    double result = 0.0;
...
void KEpsilonLidDrivenCavitySolver3D::projectToDivergenceFree(std::vector<double>& uStar,
