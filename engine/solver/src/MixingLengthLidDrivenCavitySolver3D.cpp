...
MixingLengthLidDrivenCavitySolver3D::MixingLengthLidDrivenCavitySolver3D(std::size_t nx, std::size_t ny,
    ...
    : nx_(nx), ny_(ny), nz_(nz), lengthX_(lengthX), lengthY_(lengthY), lengthZ_(lengthZ),
      ...
      p_(nx * ny * nz, 0.0), nut_(nx * ny * nz, 0.0) {}

double MixingLengthLidDrivenCavitySolver3D::u(std::size_t i, std::size_t j, std::size_t k) const {
    return u_[indexU(i, j, k)];
}
double MixingLengthLidDrivenCavitySolver3D::v(std::size_t i, std::size_t j, std::size_t k) const {
    return v_[indexV(i, j, k)];
}
double MixingLengthLidDrivenCavitySolver3D::w(std::size_t i, std::size_t j, std::size_t k) const {
    return w_[indexW(i, j, k)];
}
double MixingLengthLidDrivenCavitySolver3D::pressure(std::size_t i, std::size_t j, std::size_t k) const {
...
double MixingLengthLidDrivenCavitySolver3D::lidVelocityAt(double x, double y) const {
...
double MixingLengthLidDrivenCavitySolver3D::wallDistanceAt(std::size_t i, std::size_t j, std::size_t k) const {
...
double MixingLengthLidDrivenCavitySolver3D::stableTimeStep() const {
    double maxNut = 0.0;
    for (double n : nut_) {
...
double MixingLengthLidDrivenCavitySolver3D::uAt(long long i, long long j, long long k) const {
...
double MixingLengthLidDrivenCavitySolver3D::vAt(long long i, long long j, long long k) const {
...
double MixingLengthLidDrivenCavitySolver3D::wAt(long long i, long long j, long long k) const {
...
double MixingLengthLidDrivenCavitySolver3D::pAt(long long i, long long j, long long k) const {
    long long ci = i;
    long long cj = j;
    long long ck = k;
    if (ci < 0) {
...
double MixingLengthLidDrivenCavitySolver3D::nutAt(long long i, long long j, long long k) const {
...
double MixingLengthLidDrivenCavitySolver3D::maxDivergence() const {
    double maxDiv = 0.0;
    for (std::size_t k = 0; k < nz_; ++k) {
        for (std::size_t j = 0; j < ny_; ++j) {
            for (std::size_t i = 0; i < nx_; ++i) {
...
std::vector<double> MixingLengthLidDrivenCavitySolver3D::applyLaplacian(const std::vector<double>& x) const {
...
double MixingLengthLidDrivenCavitySolver3D::dot(const std::vector<double>& a, const std::vector<double>& b) {
    double result = 0.0;
...
void MixingLengthLidDrivenCavitySolver3D::projectToDivergenceFree(std::vector<double>& uStar,
    ...
    for (std::size_t k = 0; k < nz_; ++k) {
        for (std::size_t j = 0; j < ny_; ++j) {
            for (std::size_t i = 1; i < nx_; ++i) {
                ...
                // Effective viscosity: exact face-varying along u's own
                // (x) axis; a single cell-averaged value along the two
                // transverse (y,z) axes -- see the class comment.
