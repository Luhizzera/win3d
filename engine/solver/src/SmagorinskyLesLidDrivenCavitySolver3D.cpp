...
SmagorinskyLesLidDrivenCavitySolver3D::SmagorinskyLesLidDrivenCavitySolver3D(
    std::size_t nx, std::size_t ny, std::size_t nz, double lengthX, double lengthY, double lengthZ,
    double viscosity, double lidVelocity, double smagorinskyConstant)
    : nx_(nx), ny_(ny), nz_(nz), lengthX_(lengthX), lengthY_(lengthY), lengthZ_(lengthZ),
      ...
      smagorinskyConstant_(smagorinskyConstant), u_((nx + 1) * ny * nz, 0.0), v_(nx * (ny + 1) * nz, 0.0),
      w_(nx * ny * (nz + 1), 0.0), p_(nx * ny * nz, 0.0), nut_(nx * ny * nz, 0.0) {}

double SmagorinskyLesLidDrivenCavitySolver3D::u(std::size_t i, std::size_t j, std::size_t k) const {
    return u_[indexU(i, j, k)];
}
double SmagorinskyLesLidDrivenCavitySolver3D::v(std::size_t i, std::size_t j, std::size_t k) const {
    return v_[indexV(i, j, k)];
}
double SmagorinskyLesLidDrivenCavitySolver3D::w(std::size_t i, std::size_t j, std::size_t k) const {
    return w_[indexW(i, j, k)];
}
double SmagorinskyLesLidDrivenCavitySolver3D::pressure(std::size_t i, std::size_t j, std::size_t k) const {
    return p_[indexP(i, j, k)];
}
double SmagorinskyLesLidDrivenCavitySolver3D::subgridViscosity(std::size_t i, std::size_t j,
                                                                 std::size_t k) const {
    return nut_[indexP(i, j, k)];
}

// Delta = cbrt(dx*dy*dz), the standard filter width for an anisotropic
// cell (it reduces to the edge length on a cubic cell). This is what ties
// the model to the MESH rather than the flow geometry -- see the class
// header.
double SmagorinskyLesLidDrivenCavitySolver3D::filterWidth() const {
    return std::cbrt(dx_ * dy_ * dz_);
}

double SmagorinskyLesLidDrivenCavitySolver3D::lidVelocityAt(double x, double y) const {
...
double SmagorinskyLesLidDrivenCavitySolver3D::wallDistanceAt(std::size_t i, std::size_t j,
                                                              std::size_t k) const {
...
double SmagorinskyLesLidDrivenCavitySolver3D::stableTimeStep() const {
    double maxNut = 0.0;
    for (double n : nut_) {
...
double SmagorinskyLesLidDrivenCavitySolver3D::uAt(long long i, long long j, long long k) const {
...
double SmagorinskyLesLidDrivenCavitySolver3D::vAt(long long i, long long j, long long k) const {
...
double SmagorinskyLesLidDrivenCavitySolver3D::wAt(long long i, long long j, long long k) const {
...
double SmagorinskyLesLidDrivenCavitySolver3D::pAt(long long i, long long j, long long k) const {
    long long ci = std::clamp<long long>(i, 0, static_cast<long long>(nx_) - 1);
    long long cj = std::clamp<long long>(j, 0, static_cast<long long>(ny_) - 1);
    long long ck = std::clamp<long long>(k, 0, static_cast<long long>(nz_) - 1);
    return p_[indexP(static_cast<std::size_t>(ci), static_cast<std::size_t>(cj), static_cast<std::size_t>(ck))];
}

double SmagorinskyLesLidDrivenCavitySolver3D::nutAt(long long i, long long j, long long k) const {
...
double SmagorinskyLesLidDrivenCavitySolver3D::maxDivergence() const {
    double maxDiv = 0.0;
    for (std::size_t k = 0; k < nz_; ++k) {
        for (std::size_t j = 0; j < ny_; ++j) {
            for (std::size_t i = 0; i < nx_; ++i) {
...
std::vector<double> SmagorinskyLesLidDrivenCavitySolver3D::applyLaplacian(
...
double SmagorinskyLesLidDrivenCavitySolver3D::dot(const std::vector<double>& a,
...
void SmagorinskyLesLidDrivenCavitySolver3D::projectToDivergenceFree(std::vector<double>& uStar,
    ...
    for (std::size_t k = 0; k < nz_; ++k) {
        for (std::size_t j = 0; j < ny_; ++j) {
            for (std::size_t i = 1; i < nx_; ++i) {
                ...
                // Same locally-sampled effective viscosity treatment as
                // MixingLengthLidDrivenCavitySolver3D (exact face-varying
                // along the component's own axis, cell-averaged along the
                // two transverse axes) -- see its header for the
